...
def calculate_money_flow_profile(df, num_rows=25, source='Volume', sentiment_method='Bar Polarity'):
    ...
    for i in range(len(df)):
        ...
        # CLV-weighted bull fraction (Close-Location-Value): split each bar's
        # volume by WHERE it closed in its range instead of all-or-nothing.
        # bf=1 closed at high (full bull), bf=0 at low (full bear), 0.5 mid.
        # Matches the #138 CLV delta used by VOB build/break & _ltf_delta_volume.
        if sentiment_method == 'Bar Polarity':
            bf = (c - l) / (h - l)            # h != l guaranteed (h==l skipped)
        else:
            bf = (c - l) / (h - l)            # same CLV for buying/selling pressure
        ...
        for row in range(num_rows):
            ...
            if h >= bin_low and l < bin_high:
                ...
                if source == 'Money Flow':
                    ...
                    contrib = v * portion * mid_price
                else:
                    contrib = v * portion
                total_vol[row] += contrib
                bull_vol[row] += contrib * bf   # CLV-weighted bull share
